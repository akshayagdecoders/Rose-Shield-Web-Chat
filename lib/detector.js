...
const getAssetsDir = () => {
    ...
    // We check common locations for the public/assets directory.
    const paths = [
        path.join(process.cwd(), 'public', 'assets'),
        path.join(process.cwd(), '.next', 'server', 'public', 'assets'),
        path.join(process.cwd(), 'assets'),
        '/var/task/public/assets', // Specific to Vercel
    ];
    
    for (const p of paths) {
        try {
            // Check if directory exists and is accessible
            // (Actually just returning first since we will catch errors in loadAssets)
            return p;
        } catch (e) {}
    }
    return paths[0]; 
...
async function loadAssets() {
    try {
        console.log(`Detector: Searching for assets in ${ASSETS_DIR}`);
        ...
        try {
            ...
            try { phrasesContent = await fs.readFile(fallback, 'utf8'); } catch(e2) {
                console.warn('Keywords file not found, using empty list');
            }
        }
        ...
        try {
            ...
            console.error('Failed to load server-side AI assets:', aiErr.message);
            // Fallback: try different path if in production
            if (process.env.NODE_ENV === 'production') {
                try {
                   const fallbackModel = path.join(process.cwd(), '.next/server/public/assets/grooming_model.onnx');
                   const fallbackVocab = path.join(process.cwd(), '.next/server/public/assets/vocab.json');
                   const vocabContent = await fs.readFile(fallbackVocab, 'utf8');
                   vocab = JSON.parse(vocabContent);
                   textSession = await ort.InferenceSession.create(fallbackModel);
                   console.log('Server-side Text AI Engine Ready (via Fallback Path)');
                } catch (e) {
...
export async function checkMessage(text) {
    ...
    if (!isOffensive && textSession) {
        ...
        if (aiScore > 0.85) {
            console.log(`Server AI Blocked Message (Score: ${aiScore.toFixed(3)}): "${text.substring(0, 50)}..."`);
            isOffensive = true;
        }
    }
