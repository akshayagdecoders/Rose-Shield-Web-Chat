...
const getAssetsDir = () => {
    // lib/assets is source-tracked and bundled by Vercel
...
async function loadAssets() {
    try {
        ...
        if (!imageSession) {
            try {
                // Force trace and read into buffer 
...
export async function checkImage(buffer) {
    ...
    try {
        ...
        if (isSensitive) {
            console.warn(`Cloud AI Found Sensitive Image: ${labels[maxIdx] || maxIdx} (${(maxVal * 100).toFixed(1)}%)`);
        }
