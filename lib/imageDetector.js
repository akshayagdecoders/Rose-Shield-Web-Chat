...
export async function checkImage(buffer) {
    ...
    try {
        // 1. Resize and extract raw pixels using Jimp (Pure JS fallback for Sharp)
        const image = await Jimp.read(buffer);
        image.cover(CONFIG.INPUT_SIZE, CONFIG.INPUT_SIZE);
        ...
        image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
            floatData[floatIdx++] = (this.bitmap.data[idx + 0] / 127.5) - 1.0; // R
            floatData[floatIdx++] = (this.bitmap.data[idx + 1] / 127.5) - 1.0; // G
            floatData[floatIdx++] = (this.bitmap.data[idx + 2] / 127.5) - 1.0; // B
        });
        ...
        // Index 0 is 'Safe'
        const isSensitive = maxIdx !== 0 && maxVal > 0.6;
        
        if (isSensitive) {
            console.warn(`!!! CLOUD AI BLOCKED IMAGE: ${label} !!!`);
        }
